...
def mock_excel_data() -> pd.DataFrame:
    ...
    Returns:
        pd.DataFrame: Тестовый DataFrame.
    """
    return pd.DataFrame({"Дата операции": ["2025-03-01"], "Сумма операции": [100.0]})


@patch("pandas.read_excel")
def test_read_excel_data_success(mock_read: Any, mock_excel_data: pd.DataFrame, caplog: Any) -> None:
    """
    Тестирует успешное чтение Excel файла.

    Args:
        mock_read: Мок для pandas.read_excel.
        mock_excel_data: Тестовый DataFrame из фикстуры.
